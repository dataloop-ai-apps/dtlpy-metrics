...
from dtlpymetrics.scoring import measure_annotations
...
class ScoringAndMetrics(dl.BaseServiceRunner):
    ...
    def create_item_consensus_score(item: dl.Item) -> dl.Item:
        ...
        for annotation in annotations:
            # if annots_by_annotator.get(annotation.creator, None) is not None:
            annots_by_annotator[annotation.creator].append(annotation)
            # else:
            #     annots_by_annotator.update({annotation.creator: [annotation]})
    ...
    @staticmethod
    def create_model_score(item_set_1: list,
                           set_1_name: str = None,
                           item_set_2: list = None,
                           set_2_name: str = None,
                           ignore_labels=False):
        """
        Measures scores for a set of model predictions compared against a ground truth test set.

        :param item_set_1: list of items. When two sets are provided, this is the ground truth set
        :param set_1_name: (optional) Model name that created the first set of predictions
        :param item_set_2: (optional) if the second set of annotations are not from the same item
        :param set_2_name: Model name associated with the non-GT annotations
        :param ignore_labels: bool, True means every annotation will be cross-compared regardless of label
        :return:
        """

        gt_is_first = False if item_set_2 is None else True
        project = item_set_1[0].project
        dataset_1 = item_set_1[0].dataset
        tags = ['model evaluate', set_2_name]
        if set_1_name is not None:
            tags.append(set_1_name)

        annot_set_1 = []
        annot_set_2 = []
        if item_set_2 is None:
            if set_2_name is None:
            ...
            for item in item_set_1:
                ...
                for annotation in item.annotations.list():
                    ...
                    elif annotation.metadata['user']['model']['name'] == set_2_name:
                        item_annots_2.append(annotation)
                annot_set_1.append(item_annots_1)
                annot_set_2.append(item_annots_2)

        else:
            if len(item_set_1) != len(item_set_2):
                return False, 'Item lists are not of equal length. Please check two lists include corresponding items.'

            dataset_2 = item_set_2[0].dataset
            for item in item_set_1:
                item_annots_1 = []
                for annotation in item.annotations.list():
                    if annotation.metadata['user'].get('model', None) is None:
            ...
            for item in item_set_2:
                ...
                for annotation in item.annotations.list():
                    if annotation.metadata['user'].get('model', None) is not None:
                        if annotation.metadata['user']['model']['name'] == set_2_name:
                            item_annots_2.append(annotation)
                annot_set_2.append(item_annots_2)

        if not item_annots_1:
            return False, 'No ground truth annotations found in the first set. ' \
                          'Please ensure there are annotations in the items.'

        all_results = pd.DataFrame()
        for i in range(len(item_set_1)):
            ...
            if gt_is_first:
                results_df['item_1_id'] = item_set_1[i].id
                results_df['item_2_id'] = item_set_2[i].id if gt_is_first else item_set_1[i].id

            all_results = pd.concat([all_results, results_df],
                                    ignore_index=True)

        feature_set_prefix = 'Model ' + set_2_name + ' '
        score_sets = {}
        for score_name in score_names:
            try:
                ...
                feature_set = project.feature_sets.create(name=f'{feature_set_prefix}{score_name}', #12345678901234567
                                                          set_type='scores',
            ...
            for score, feature_set in score_sets.items():
                if not gt_is_first:
                    if row['first_id'] is not None:
                        # if 'bot.dataloop.ai' in row['first_creator']: # to mark it as a model prediction
                        ...
                        feature1 = feature_set.features.create(value=[row[results_columns[score.lower()]]],
                                                               ...
                                                               refs={'item': row['item_id'],
                                                                     # 'model': set_1_name, # TODO what is the right ref?
                ...
                if row['second_id'] is not None:
                    feature2 = feature_set.features.create(value=[row[results_columns[score.lower()]]],
                                                           ...
                                                           refs={'item': row['item_id'],
                                                                 # 'model': set_2_name, # TODO what is the right ref?
                                                                 'dataset': dataset_2.id,
                                                                 'relative': row['first_id'],
                                                                 })

        return True, annot_set_1, annot_set_2
...
if __name__ == '__main__':
    ...
    # item = dl.items.get(None, '6450ca8032fe78dcdf9e8e47')
    # items_list = [item]

    success, message = ScoringAndMetrics.create_model_score(item_set_1=items_list,
                                                            set_2_name='cloned_yolo_demo')
    # ignore_labels=True)
