...
def get_consensus_score(consensus_task, save_plot=False):
    ...
    for item in items:
        ...
        if item.metadata['system']['refs'][0]['metadata'][
            ...
            if save_plot:
                ...
                plot_matrix(item_title, filename, annotator_agreement, annotators, item)

            # calculate the average agreement across all annotators for the lower half of the matrix, without the diagonal
    ...
    plot_matrix(consensus_title, filename, mean_task_consensus, annotators)
...
if __name__ == '__main__':
    ...
    feature_set = get_consensus_score(consensus_task, save_plot=SAVE_PLOT)
