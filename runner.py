...
class Scorer(dl.BaseServiceRunner):
    ...
    def consensus_agreement(item: dl.Item, context: dl.Context, progress: dl.Progress, task: dl.Task = None) -> dl.Item:
        ...
        node = context.node
        agreement_config["agreement_threshold"] = node.metadata.get("customNodeConfig", dict()).get("agreement_threshold", 0.5)
    ...
    def model_agreement(item: dl.Item, model: dl.Model, context: dl.Context, progress: dl.Progress) -> dl.Item:
        ...
        node = context.node
        agreement_config["agreement_threshold"] = node.metadata.get("customNodeConfig", dict()).get("agreement_threshold", 0.5)
        agreement_config["keep_annots"] = node.metadata.get("customNodeConfig", dict()).get("model_keep_annots", True)
